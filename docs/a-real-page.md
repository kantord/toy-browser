...
It was **forced synchronous layout**: the page asked how big something was 300
times while building itself, and each answer re-parsed and re-cascaded the
whole document. Keeping the last answer took the load from 25.6s to 4.3s.

That has its own document, because it is about how a page is measured rather
than about what this page needed: `docs/measuring-again.md`, which also has
the measurement showing why keeping *more* than the last answer buys one extra
hit in a whole page load.
