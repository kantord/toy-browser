...
fn going_back_returns_to_the_page_before() {
    let mut browser = browser();
    let page = browser.new_page().unwrap();
    browser.navigate(&page, fixture("webview-top.html").as_str()).unwrap();
    browser.navigate(&page, fixture("webview-elsewhere.html").as_str()).unwrap();
...
fn going_back_walks_back_one_at_a_time() {
    let mut browser = browser();
    let page = browser.new_page().unwrap();
    for name in ["webview-top.html", "webview-bottom.html", "webview-elsewhere.html"] {
    ...
    let ends: Vec<&str> = walked.iter().map(|at| at.rsplit('/').next().unwrap()).collect();
    // `about:blank` is where a new page starts, and it is behind everything.
    assert_eq!(ends, ["webview-bottom.html", "webview-top.html", "about:blank"]);
}
...
fn a_chrome_page_says_which_page_it_is_about() {
    let mut browser = browser();
    let page = browser.new_page().unwrap();
    browser.navigate(&page, fixture("webview-host.html").as_str()).unwrap();
