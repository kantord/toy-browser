...
///
/// A page asks this the same way a front end does, and both are served from the
/// measure already published — which is why hit testing lives in the engine at
/// all, rather than beside the boxes in the layer that produced them.
pub(super) fn element_from_point(ctx: &Ctx<'_>, x: f64, y: f64) -> rquickjs::Result<Option<usize>> {
    let shared = ctx
        .userdata::<Sharing>()
        .ok_or_else(|| rquickjs::Error::new_from_js("Realm", "a document to belong to"))?;
    Ok(shared.hit(Point {
